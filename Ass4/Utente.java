...
public class Utente implements Runnable {
    ...
    private int index;
    private MonitorLab Lab;
    private final int minDelay = 200;
    private final int maxDelay = 1000;

    public Utente(int i, MonitorLab Lab, String ruolo){
        this.index = i;
        this.Lab = Lab;
        this.ruolo = ruolo;
    }
    ...
    public void run(){
        ...
        if ( this.ruolo.compareTo("Studente") == 0 ){
            ...
            try { 
                ...
                if ( pc < 0 ){
                    System.out.printf("Studente - Oh, oh, something wrong, I can feel it\n");
                    return;
                }
                System.out.printf("Studente - Occupo il PC %d\n", pc);
                Thread.sleep(ThreadLocalRandom.current().nextInt(minDelay, maxDelay));
                System.out.printf("Studente - Esco nel laboratorio (lascio libero il PC %d)\n", pc);
                Lab.freePC(pc);
            }
            catch ( InterruptedException e ){
